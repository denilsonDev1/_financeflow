...
export class UserController {
    constructor(private createUserUseCase: CreateUserUseCase, private findUserByIdUseCase: FindUserByIdUseCase){
    }
    ...
    public async index(req: FastifyRequest<{Params: {id: string}}>, res: FastifyReply){
        const users = await this.findUserByIdUseCase.execute(req.params.id)
        if(!users.success) return res.code(500).send({error: "server internal error"})
